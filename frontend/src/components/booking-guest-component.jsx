/* Description: This component is intended for Admin reserving 
requests for guest.
...
const BookGuestComponent = () => {
  ...
  /*
    Description: Fetch room and guest list from database
  ...
  */
  useEffect(() => {
    const fetchRoomsList = async () => {
      try {
        ...
        const sorted_rooms = rooms.sort((a, b) => {  // sort by room name
          // convert room names to lowercase for case-insensitive sorting
        ...
        // creates objects for the room infos
    ...
    const fetchAllGuests = async () => {
      try {
        const get_guests = await api.getAllGuests();
        // initialize guest info by default 
        const guests = [{
          first_name: '---',
          middle_name: '',
          last_name: ''
        }]
        // append the fetched data to the initialized list
        guests.push(...get_guests.data.guests)
        setGuestList(guests);
      } catch (error) {
  ...
  return (
    <>
      ...
      <div>
        <title>Book Guest Reservation | PICSEL</title>
        <div className="page-description">
          <h1>Book Guest Reservation</h1>
        </div>
            ...
            <div className="widget-stats-content flex-fill" >
              ...
              {/* Form Section Start */}
                <form className="room-rental-for" onSubmit={handleSubmit}>
                  ...
                  <br></br>
                  <div className='form-group' >
                    <h3 className='gray-text'>Guest</h3>
                    <select class="js-states form-control" tabindex="-1"
                      style={{ height: 40 }}
                      id='guest'
                      value={guest}
                    ...
                    >
                      {guestList.map((guest) => (
                        <option key={guest.user_id} value={guest.user_id}>
                          {`${guest.first_name} ${guest.middle_name !== 'N/A' ? guest.middle_name + ' ' : ''}${guest.last_name}`}
                        </option>
                      ))}
                    </select>
                  </div>
                  ...
                  <button type="submit" className="btn btn-primary"
                    disabled={room === '---' || guest === '---' || startTime === '--:--' || endTime === '--:--' || date === '' || eventTitle === '' || purpose.trim() === '' || characterLimitExceeded || eventTitle.trim() === '' || eventTitleCharacterLimitExceeded}
                  >
...
export default BookGuestComponent;